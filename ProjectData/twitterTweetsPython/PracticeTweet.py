...
#   username=ktrafficpolicee;
    ...
    #get tweets
    tweets = api.user_timeline(screen_name = 'khiTraffic',count = number_of_tweets,tweet_mode='extended')
    
    #create array of tweet information: username, tweet id, date/time, text
    tweets_for_csv = [['khiTraffic',tweet.id_str, tweet.created_at, tweet.full_text] for tweet in tweets]
    i=0
...
#    print("number1")
    #another account
    #get tweets
    tweets = api.user_timeline(screen_name = 'ktrafficpolicee',count = number_of_tweets,tweet_mode='extended')
    
    #create array of tweet information: username, tweet id, date/time, text
#    print(tweets[0][1])
    tweets_for_csv = [['ktrafficpolicee',tweet.id_str, tweet.created_at, tweet.full_text] for tweet in tweets]
    i=0
    while i<len(tweets_for_csv):
        cursor.execute("Insert into twitter(created_at, tweet) values (%s,%s)",(tweets_for_csv[i][2],tweets_for_csv[i][3]))
        i+=1
